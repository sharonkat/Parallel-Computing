...
int main(int argc, char* argv[]) {
    ...
    // Definitions
    char *new_list, *d_new_list, *old_list, *d_old_list;
    // Sizing values
    ...
    old_list = readFile(file_path, number_of_lines);

    clock_t start_time = clock();

    // Malloc array for results
    new_list = (char*)malloc(number_of_lines * sizeof(char*));

    // Allocation memory on GPU for old list and new list
    cudaMalloc(&d_old_list, number_of_vals*sizeof(char*));
    cudaMalloc(&d_new_list, number_of_lines*sizeof(char*));

    // Copy old list to the GPU
    cudaMemcpy(d_old_list, old_list, (number_of_vals*sizeof(char*)), cudaMemcpyHostToDevice);
    ...
    // Launch kernel
    logic_gates<<<num_threads, num_blocks>>>(num_threads, num_blocks, d_old_list, d_new_list);

    // Copy new list to the CPU (size = #lines)
    cudaMemcpy(new_list, d_new_list, (number_of_lines*sizeof(char*)), cudaMemcpyDeviceToHost);
    ...
    if(fp != NULL)
    {
        ...
        while(fgets(line, sizeof line, fp) != NULL)
        {
            ...
            while( token != NULL ) {
                ...
                list_of_vals[array_counter] = atoi(token);
                array_counter++;
